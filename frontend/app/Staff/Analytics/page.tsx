          ...
          {Object.entries(statusCounts)
            ...
            .map(([name, count]) => (
              <StatCard
                ...
                sub={name.toLowerCase().includes("pending")?"Needs attention":name.toLowerCase().includes("inspect")?"Under inspection":name.toLowerCase().includes("progress")?"Work in progress":name.toLowerCase().includes("resolved")?`${filtered.length>0?Math.round((count/filtered.length)*100):0}% of total`:name.toLowerCase().includes("unfinished")?`${filtered.length>0?Math.round((count/filtered.length)*100):0}% of total`:name.toLowerCase().includes("closed")?"Completed & closed":""}
          ...
          <div className="card analytics-card-full">
            <div className="resolution-head">
              <div>
                ...
                <p className="muted" style={{margin:"4px 0 0"}}>{statusCounts["Resolved"]||0} resolved of {filtered.length} total</p>
              </div>
