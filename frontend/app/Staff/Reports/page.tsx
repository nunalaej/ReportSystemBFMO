...
// ─── Self-contained — no external hook imports needed ────────────
// Features matching Admin Reports:
//   ✓ Card + List view toggle
//   ✓ Progress modal (stepper + history timeline)
//   ✓ Create Task full modal (priority, checklist, staff assign)
//   ✓ View Task → /Staff/Task?highlight=id
//   ✓ Date filter row (gated by canFilterDates perm)
//   ✓ Duplicate grouping + pagination
//   ✓ Print with signatures
//   ✓ All permission gates from positionPerms DB
// New — concern-based visibility:
//   ✓ Staff see only reports matching their disciplines
//   ✓ "Other" concern always visible to all staff
//   ✓ Staff with NO disciplines set → sees everything
// ─────────────────────────────────────────────────────────────────
"use client";
...
/* ══════════════════════════════════════════════════════════
   PERMISSION SYSTEM (loaded from DB via api/meta → positionPerms)
══════════════════════════════════════════════════════════ */
type Perms = {
...
const BASE_PERMS: Perms = {
  canViewReports:true, canViewProgress:true, canViewTask:true,
  canFilterDates:false, canComment:false, canUpdateStatus:false,
  canArchive:false, canCreateTask:false,
};
function buildPerms(permList: string[]): Perms {
  ...
  const hasReal = set.has("update status") || set.has("comment") || set.has("create tasks") ||
                  set.has("edit tasks")    || set.has("assign staff") || set.has("delete tasks") ||
  ...
  return {
    ...
    canArchive:      set.has("update status"),
...
function isWithinDateRange(ds: string|undefined, f: DateFilter, from: string, to: string): boolean {
  ...
  if (f === "custom") {
    const a = from ? new Date(from) : null; const b = to ? new Date(to+"T23:59:59") : null;
    if (a && date < a) return false; if (b && date > b) return false; return true;
  }
  return true;
}
const getGroupKey = (r: Report) => {
  const b=(r.building||"").trim(); const c=(r.concern||"").trim();
  const sc=(r.subConcern||r.otherConcern||"").trim(); const rm=(r.room||r.otherRoom||"").trim();
...
const FALLBACK_STATUSES: MetaStatus[] = [
  {id:"1",name:"Pending",color:"#FFA500"},{id:"2",name:"Pending Inspect",color:"#FFD700"},
  {id:"3",name:"In Progress",color:"#4169E1"},{id:"4",name:"Resolved",color:"#28A745"},{id:"5",name:"Archived",color:"#6C757D"},
];
const FALLBACK_PRIORITIES: MetaPriority[] = [
  {id:"1",name:"Low",color:"#28A745"},{id:"2",name:"Medium",color:"#FFC107"},
  {id:"3",name:"High",color:"#ce4f01"},{id:"4",name:"Urgent",color:"#a40010"},
];
...
function useToast() {
  ...
  const show = useCallback((message: string, type: ToastType = "success") => {
    const id = ++toastId; setToasts(p=>[...p,{id,message,type}]);
...
function useEscapeKey(handler: ()=>void, active: boolean) {
  useEffect(()=>{
    ...
    window.addEventListener("keydown",l); return ()=>window.removeEventListener("keydown",l);
  },[handler,active]);
}

/* ══════════════════════════════════════════════════════════
   INLINE STAFF PERMS HOOK
══════════════════════════════════════════════════════════ */
function useStaffPerms(clerkUserId?: string) {
  ...
  const permSummary = ()=>{
    ...
    const posLower = (staffRecord?.position||"").trim().toLowerCase();
    const entry    = Object.entries(positionPerms).find(([k])=>k.trim().toLowerCase()===posLower);
    const permList: string[] = Array.isArray(entry?.[1])?entry![1].filter((v):v is string=>typeof v==="string"):[];
...
export default function StaffReportsPage() {
  ...
  useEffect(()=>{
    ...
    fetch(`${API_BASE}/api/staff`,{cache:"no-store"}).then(r=>r.json()).catch(()=>null)
      .then(data=>{
  ...
  const fetchTasks = useCallback(async()=>{
    try {
      ...
      const list:LinkedTask[]=Array.isArray(data)?data:Array.isArray(data.tasks)?data.tasks:[];
      setAllTasks(list);
    } catch {}
  },[]);
  ...
  /* ══════════════════════════════════════════════════════════
     DISCIPLINE-BASED VISIBILITY
     Staff see only reports matching their disciplines.
     "Other" concern is ALWAYS visible to all staff.
     Staff with NO disciplines assigned see everything.
  ══════════════════════════════════════════════════════════ */
  const staffDisciplines=useMemo(()=>(staffRecord?.disciplines||[]).map(d=>d.toLowerCase().trim()),[staffRecord]);

  const canSeeReport=useCallback((r:Report):boolean=>{
    const concern=(r.concern||"").toLowerCase().trim();
    if (!concern||concern==="other") return true;  // Other → always visible
    if (staffDisciplines.length===0) return true;  // No disciplines → sees all
    return staffDisciplines.some(d=>concern.includes(d)||d.includes(concern));
  },[staffDisciplines]);

  const getLinkedTask=useCallback((r:Report):LinkedTask|null=>{
    if (!r.reportId) return null;
    return allTasks.find(t=>t.reportId===r.reportId)??null;
  },[allTasks]);
  ...
  /* ── Visible reports (discipline-filtered) ── */
  const visibleReports=useMemo(()=>reportsToDisplay.filter(canSeeReport),[reportsToDisplay,canSeeReport]);

  /* ── Filter options ── */
  const buildingOptions=useMemo(()=>["All Buildings",...new Set(visibleReports.filter(r=>(concernFilter==="All Concerns"||r.concern===concernFilter)&&(collegeFilter==="All Colleges"||(r.college||"Unspecified")===collegeFilter)&&statusMatchesFilter(r.status,statusFilter)).map(r=>r.building).filter((v):v is string=>Boolean(v)))]  ,[visibleReports,concernFilter,collegeFilter,statusFilter,statusMatchesFilter]);
  const concernOptions =useMemo(()=>["All Concerns",...new Set(visibleReports.filter(r=>(buildingFilter==="All Buildings"||r.building===buildingFilter)&&(collegeFilter==="All Colleges"||(r.college||"Unspecified")===collegeFilter)&&statusMatchesFilter(r.status,statusFilter)).map(r=>r.concern).filter((v):v is string=>Boolean(v)))]  ,[visibleReports,buildingFilter,collegeFilter,statusFilter,statusMatchesFilter]);
  ...
  useEffect(()=>{
    const valid=new Set(visibleReports.filter(r=>(buildingFilter==="All Buildings"||r.building===buildingFilter)&&(collegeFilter==="All Colleges"||(r.college||"Unspecified")===collegeFilter)&&statusMatchesFilter(r.status,statusFilter)).map(r=>r.concern));
    if (concernFilter!=="All Concerns"&&!valid.has(concernFilter)) setConcernFilter("All Concerns");
  },[buildingFilter,collegeFilter,statusFilter,visibleReports,concernFilter]);

  useEffect(()=>{
    const valid=new Set(visibleReports.filter(r=>(concernFilter==="All Concerns"||r.concern===concernFilter)&&(collegeFilter==="All Colleges"||(r.college||"Unspecified")===collegeFilter)&&statusMatchesFilter(r.status,statusFilter)).map(r=>r.building));
    if (buildingFilter!=="All Buildings"&&!valid.has(buildingFilter)) setBuildingFilter("All Buildings");
  },[concernFilter,collegeFilter,statusFilter,visibleReports,buildingFilter]);
  ...
  const handlePrint=useCallback(()=>{
    ...
    const cBase=new Map<string,number>();const bMap=new Map<string,number>();
    ...
    <h2>By Concern</h2><ul>${[...cBase.entries()].sort((a,b)=>b[1]-a[1]).map(([n,c])=>`<li>${n}: ${c}</li>`).join("")||"<li>No data.</li>"}</ul>
    <h2>By Building</h2><ul>${[...bMap.entries()].sort((a,b)=>b[1]-a[1]).map(([n,c])=>`<li>${n}: ${c}</li>`).join("")||"<li>No data.</li>"}</ul>
    <h2>Detailed Report</h2><table><thead><tr><th>#</th><th>Report ID</th><th>Date Created</th><th>Status</th><th>Building</th><th>Concern</th><th>College</th><th>Floor</th><th>Room</th><th>Reporter Type</th></tr></thead><tbody>${rows||'<tr><td colspan="11">No data.</td></tr>'}</tbody></table>
    ...
    <div className="progress-backdrop" onClick={()=>{setShowProgress(false);setProgressReport(null);}}>
      <div className="progress-modal" onClick={e=>e.stopPropagation()}>
        <div className="progress-modal-header">
          <div><h2 className="progress-modal-title">Report Progress</h2>{progressReport.reportId&&<p className="progress-modal-sub">#{progressReport.reportId} · {progressReport.heading}</p>}</div>
                ...
                return(
                  <div key={step.id} className={`stepper-step ${stateClass}`}>
                    ...
                    <div className="stepper-content">
                      ...
                      <span className="stepper-status" style={state!=="pending"?{backgroundColor:step.color+"22",color:step.color}:{}}>{state==="completed"?"Completed":state==="active"?"Current":"Pending"}</span>
                    </div>
                  </div>
                );
              })}
                    ...
                    return(
                      <div key={idx} className="progress-timeline-item">
                        ...
                        <div className="progress-timeline-content">
                          ...
                          <div className="progress-timeline-meta">{entry.by&&<span>by {entry.by}</span>}<span>{entry.at?new Date(entry.at).toLocaleString():""}</span></div>
                        </div>
                      </div>
                    );
                  })}
    ...
    <div className="report-modal-backdrop" onClick={closeDetails} role="dialog" aria-modal="true">
      <div className="report-modal" onClick={e=>e.stopPropagation()}>
        <div className="modal-header">
          <div className="modal-header-main">
            ...
            {staffRecord&&<span style={{fontSize:"0.62rem",fontWeight:700,padding:"2px 8px",borderRadius:999,background:rb.bg,color:rb.color,textTransform:"uppercase",letterSpacing:"0.05em",marginLeft:4}}>{staffRecord.position}</span>}
          </div>
          <div className="modal-header-actions">
            <button type="button" className="modal-progress-btn" onClick={()=>openProgress(selectedReport)}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>Progress
            </button>
        ...
        <div className="modal-content">
          <div className="modal-img-wrapper">
            <img src={resolveImg(selectedReport.ImageFile||selectedReport.image)} alt="Report" className="report-img report-img-clickable" onClick={()=>setIsImageExpanded(true)} onError={e=>{(e.target as HTMLImageElement).src=defaultImg;}}/>
          ...
          <div className="modal-info">
            ...
            <div className="status-panel" style={{borderLeft:`3px solid ${getStatusColor(statusValue)}`}}>
              ...
              {perms.canUpdateStatus?(
                <div className="status-row status-row-inline">
                  ...
                  <select id="status-select" className="status-select" value={statusValue} onChange={e=>setStatusValue(e.target.value)} disabled={selectedReport.status==="Archived"}>
                    {metaStatuses.filter(s=>s.name.toLowerCase()!=="archived").map(s=><option key={s.id} value={s.name}>{s.name}</option>)}
                  </select>
                </div>
              ):<p style={{fontSize:"0.72rem",color:"var(--tasks-text-4,#b8c4ce)",marginTop:6}}>Your role ({staffRecord?.position}) cannot update status.</p>}
            </div>
            <div className="comments-section">
                ...
                <ul className="comments-list">
                  {commentsToShow.map((c,idx)=>(
                    <li key={idx} className="comment-item">
                      {editingIndex===idx?(
                        <><textarea className="comment-edit-input" rows={2} value={editingText} onChange={e=>setEditingText(e.target.value)} autoFocus/><div className="comment-actions-row"><button type="button" className="comment-btn-save" onClick={()=>saveEditedComment(idx)} disabled={saving}>Save</button><button type="button" className="comment-btn-cancel" onClick={cancelEditComment} disabled={saving}>Cancel</button></div></>
                      ):(
                        <><p className="comment-text">{c.text||c.comment||String(c)}</p>{c.imageUrl&&<img src={c.imageUrl} alt="attachment" className="comment-image" onError={e=>{(e.target as HTMLImageElement).style.display="none";}}/>}
                        <div className="comment-footer">
                          <div>{c.at&&<span className="comment-date">{new Date(c.at).toLocaleString()}&nbsp;</span>}{c.by&&<span className="comment-date">by {c.by}</span>}</div>
                          {perms.canComment&&<div className="comment-actions"><button type="button" className="comment-btn-edit" onClick={()=>startEditComment(idx)} disabled={saving}>Edit</button><button type="button" className="comment-btn-delete" onClick={()=>deleteComment(idx)} disabled={saving}>Delete</button></div>}
                        </div></>
                      )}
                    </li>
                  ))}
              ...
              {perms.canComment?(
                <><textarea className="comment-input" rows={3} value={commentText} onChange={e=>setCommentText(e.target.value)} placeholder="Type your comment here…"/>
                <div className="modal-actions">
                  ...
                  {perms.canArchive&&selectedReport.status!=="Archived"&&<button className="archive-btn" onClick={handleArchive} disabled={saving} type="button">Archive report</button>}
                  {perms.canUpdateStatus&&<button className="save-comment-btn" onClick={handleSaveChanges} disabled={saving||selectedReport.status==="Archived"} type="button">{saving?"Updating…":"Update Status"}</button>}
                </div></>
              ):(
                <div style={{marginTop:10,padding:"10px 14px",background:"rgba(0,0,0,0.03)",borderRadius:8,border:"1px solid var(--border,#e8ecf0)"}}>
                  <p style={{fontSize:"0.78rem",color:"var(--tasks-text-4,#b8c4ce)",margin:0}}>💬 Comments are read-only for your role ({staffRecord?.position}).</p>
                  {(perms.canUpdateStatus||perms.canArchive)&&<div className="modal-actions" style={{marginTop:10}}>
                    {perms.canArchive&&selectedReport.status!=="Archived"&&<button className="archive-btn" onClick={handleArchive} disabled={saving} type="button">Archive report</button>}
                    {perms.canUpdateStatus&&<button className="save-comment-btn" onClick={handleSaveChanges} disabled={saving||selectedReport.status==="Archived"} type="button">{saving?"Updating…":"Update Status"}</button>}
                  </div>}
                </div>
      ...
      {isImageExpanded&&(
        <div className="image-fullscreen-backdrop" onClick={()=>setIsImageExpanded(false)}>
          <img src={resolveImg(selectedReport.ImageFile||selectedReport.image)} alt="Report full view" className="image-fullscreen-img" onClick={e=>e.stopPropagation()} onError={e=>{(e.target as HTMLImageElement).src=defaultImg;}}/>
  ...
  const taskModalContent=showTaskModal&&taskReport&&perms.canCreateTask?(
    <div className="report-modal-backdrop" onClick={closeTaskModal} role="dialog" aria-modal="true">
      <div className="task-modal" onClick={e=>e.stopPropagation()}>
        <div className="modal-header">
          <div className="modal-header-main"><h2>Create Task</h2>{taskReport.reportId&&<span className="modal-report-id-badge">#{taskReport.reportId}</span>}</div>
        ...
        <div className="task-modal-body">
            ...
            <div className="task-field-group">
              ...
              <div className="task-priority-grid">
                {metaPriorities.map(p=>(
                  <button key={p.id} type="button" className={`task-priority-btn${taskPriority===p.id?" task-priority-btn--active":""}`}
          ...
          <div className="task-modal-right">
            <div className="task-field-group">
              ...
              {taskStaff.length>0&&<div className="task-staff-tags">{taskStaff.map(name=>(<span key={name} className="task-staff-tag"><span className="task-staff-avatar">{name.charAt(0).toUpperCase()}</span>{name}<button type="button" className="task-staff-remove" onClick={()=>removeTaskStaff(name)}>✕</button></span>))}</div>}
              <div className="task-staff-input-row">
                {metaStaff.length>0?(
                  <select className="task-input" value={taskStaffInput} onChange={e=>setTaskStaffInput(e.target.value)}>
                    ...
                    {metaStaff.filter(s=>!taskStaff.includes(s.name)&&(s.disciplines.length===0||s.disciplines.some(d=>d.toLowerCase()===(taskReport?.concern||"").toLowerCase()))).map(s=><option key={s.name} value={s.name}>{s.name}</option>)}
                  </select>
                ):(
                  <input type="text" className="task-input" value={taskStaffInput} placeholder="Staff name or email…" onChange={e=>setTaskStaffInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();addTaskStaff();}}}/>
                )}
                <button type="button" className="task-add-btn" onClick={addTaskStaff} disabled={!taskStaffInput.trim()}>Add</button>
              </div>
            </div>
            <div className="task-field-group">
              <label className="task-label">Progress Checklist {taskChecklist.length>0&&<span className="task-checklist-count">{taskChecklist.filter(i=>i.done).length}/{taskChecklist.length}</span>}</label>
              {taskChecklist.length>0&&<div className="task-progress-bar-wrap"><div className="task-progress-bar-fill" style={{width:`${Math.round((taskChecklist.filter(i=>i.done).length/taskChecklist.length)*100)}%`}}/></div>}
              <div className="task-checklist-list">
                {taskChecklist.map(item=>(<div key={item.id} className="task-checklist-item"><button type="button" className={`task-check-btn${item.done?" task-check-btn--done":""}`} onClick={()=>toggleCheckItem(item.id)}>{item.done&&<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>}</button><span className={`task-checklist-text${item.done?" task-checklist-text--done":""}`}>{item.text}</span><button type="button" className="task-checklist-remove" onClick={()=>removeCheckItem(item.id)}>✕</button></div>))}
                {taskChecklist.length===0&&<p className="task-checklist-empty">No items yet.</p>}
              </div>
              <div className="task-staff-input-row">
                <input type="text" className="task-input" value={taskCheckInput} placeholder="Add a checklist step…" onChange={e=>setTaskCheckInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();addChecklistItem();}}}/>
        ...
        <div className="task-modal-footer">
          ...
          <button type="button" className="task-submit-btn" onClick={handleCreateTask} disabled={taskSaving||!taskName.trim()}>{taskSaving?"Creating…":"Create Task"}</button>
        </div>
      </div>
    </div>
  ):null;
  ...
  const renderReportCard=(report:Report,showDupLink=false)=>{
    ...
    const statusKey=getStatusClassKey(report.status); const existing=getLinkedTask(report);
    return(
      <div key={report._id} className="report" onClick={()=>handleCardClick(report)}>
        ...
        <div className="report-img-container" style={{position:"relative"}}>
          <img src={resolveImg(report.image||report.ImageFile)} alt="Report" className="report-img" onError={e=>{(e.target as HTMLImageElement).src=defaultImg;}}/>
          {existing&&<span style={{position:"absolute",top:6,right:6,background:"#16a34a",color:"#fff",fontSize:"0.6rem",fontWeight:700,padding:"2px 6px",borderRadius:999}}>✓ Task</span>}
        </div>
        <div className="report-body">
          ...
          <div className={`status-focus-row status-focus-${statusKey}`}><span className="status-focus-label">Status</span>{renderStatusPill(report.status)}</div>
          ...
          <div className="report-card-actions" onClick={e=>e.stopPropagation()}>
            <button type="button" className="card-progress-btn" onClick={e=>openProgress(report,e)}>
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>Progress
            </button>
        ...
        <div className="header">
          <div>
            ...
            <p className="header-subtitle" style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
              ...
              {staffRecord&&(<>
                <span style={{background:rb.bg,color:rb.color,fontSize:"0.68rem",fontWeight:700,padding:"2px 8px",borderRadius:999}}>{staffRecord.position}</span>
                {permsLoaded&&<span style={{fontSize:"0.65rem",color:"var(--tasks-text-4,#b8c4ce)",fontStyle:"italic"}}>{permSummary()}</span>}
                {staffDisciplines.length>0&&<span style={{fontSize:"0.65rem",background:"#f0f9ff",color:"#0369a1",border:"1px solid #bae6fd",borderRadius:999,padding:"1px 8px",fontWeight:600}}>Showing: {staffRecord.disciplines.join(", ")} + Other</span>}
              </>)}
            </p>
          </div>
          <div className="header-actions">
            ...
            <button className="refresh-btn" type="button" onClick={()=>{fetchReports();fetchTasks();}} disabled={isLoading} title="Refresh">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="16" height="16"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>
            </button>
        ...
        <div className="filters-card">
          ...
          {perms.canFilterDates?(
            ...
            <div style={{marginTop:10,padding:"8px 12px",background:"rgba(0,0,0,0.03)",borderRadius:8,border:"1px solid var(--border,#e8ecf0)",display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:"0.75rem",color:"var(--tasks-text-4,#b8c4ce)"}}>Date filtering available for roles with update permissions.</span>
            </div>
          )}
        </div>
          ...
          <>
            {selectedGroup?(
              <div className={viewMode==="list"?"reports-list-view":"reports-list"}>
                ...
                {viewMode==="list"?(<><div className="report-list-header"><span>Report</span><span>Status</span><span>Building</span><span>Concern</span><span>College</span><span>Date</span><span>Actions</span></div>{getReportsByGroup(selectedGroup).map(r=>renderReportRow(r))}</>):getReportsByGroup(selectedGroup).map(r=>renderReportCard(r,false))}
              </div>
            ):(
              <>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                  <span style={{fontSize:"0.8rem",color:"#6b7280"}}>{filteredReports.length} report{filteredReports.length!==1?"s":""}{dateFilter!=="all"&&perms.canFilterDates&&` · ${dateFilter==="today"?"Today":dateFilter==="week"?"Last 7 days":dateFilter==="month"?"Last 30 days":"Custom range"}`}</span>
                </div>
                {viewMode==="list"?(
