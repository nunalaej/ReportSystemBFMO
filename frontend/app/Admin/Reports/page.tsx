...
const formatConcernLabel = (r: Report) => {
  const base = r.concern || "Unspecified";
  const sub  = r.subConcern || r.otherConcern;
  return sub ? `${base} : ${sub}` : base;
};
...
useEffect(() => {
  fetch(`${API_BASE}/api/meta?ts=${Date.now()}`, { cache:"no-store" })
    ...
    .then(data => {
      ...
      // ✅ Load signatories from database
  ...
  const signatoryModalContent = showPrintModal ? (
    <div className="report-modal-backdrop" onClick={() => setShowPrintModal(false)}>
      <div className="signatory-modal" onClick={e => e.stopPropagation()} style={{
        background: "white",
        borderRadius: "16px",
        padding: "24px",
        width: "90%",
        ...
        <p style={{ color: "#666", marginBottom: "20px", fontSize: "14px" }}>
          Add or edit signatory names that will appear on the printed report.
        </p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px", marginBottom: "24px" }}>
          {signatories.map((sig, idx) => (
            <div key={idx} style={{ 
              ...
              <input
                type="text"
                placeholder="Role (e.g., Head Engineer)"
                value={sig.role}
                onChange={(e) => {
                  const updated = [...signatories];
              ...
              />
              <button
                onClick={() => setSignatories(signatories.filter((_, i) => i !== idx))}
                style={{
                  padding: "8px 12px",
                  background: "#fee2e2",
                  border: "none",
        ...
        <button
          onClick={() => setSignatories([...signatories, { name: "", role: "" }])}
          style={{
            width: "100%",
            padding: "10px",
            background: "#f3f4f6",
...
const yearOptions = ["All Years", ...new Set(reports.filter(r =>
  ...
  const executePrint = useCallback((sigs: Signatory[]) => {
    ...
    const rowsHtml = filteredReports.map((r, i) =>
      `<tr><td style="border:1px solid #d1d5db;padding:4px 6px;">${i + 1}</td><td style="border:1px solid #d1d5db;padding:4px 6px;">${safe(r.reportId)}</td><td style="border:1px solid #d1d5db;padding:4px 6px;">${r.createdAt ? new Date(r.createdAt).toLocaleString() : ""}</td><td style="border:1px solid #d1d5db;padding:4px 6px;">${safe(r.status)}</td><td style="border:1px solid #d1d5db;padding:4px 6px;">${safe(r.building)}</td><td style="border:1px solid #d1d5db;padding:4px 6px;">${safe(formatConcernLabel(r))}</td><td style="border:1px solid #d1d5db;padding:4px 6px;">${safe(r.college)}</td><td style="border:1px solid #d1d5db;padding:4px 6px;">${safe(r.floor)}</td><td style="border:1px solid #d1d5db;padding:4px 6px;">${safe(r.room)}</td><td style="border:1px solid #d1d5db;padding:4px 6px;">${safe(r.email)}</td><td style="border:1px solid #d1d5db;padding:4px 6px;">${safe(r.userType)}</td></tr>`
    ).join("");
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"/><title>BFMO Analytics</title><style>
      ...
      .title{font-size:14px;font-weight:700;color:#fff;background:#029006;padding:8px}
      ...
      li{margin:2px 0}
      .sig-row{display:flex;justify-content:space-around;gap:24px;flex-wrap:wrap;margin-top:48px}
      .sig-block{flex:1;min-width:140px;max-width:200px;text-align:center}
      .sig-line{border-top:1px solid #111827;margin-top:40px;margin-bottom:4px}
      .sig-name{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.04em}
      .sig-role{font-size:8px;color:#6b7280;margin-top:2px}
      @media print{*{-webkit-print-color-adjust:exact!important}}
    </style></head><body>
    <table class="doc-table"><tr><td class="logo-cell" rowspan="4"><img src="/logo-dlsud.png" style="width:64px;height:64px;object-fit:contain"/></td><td colspan="2" class="title">Building Facilities Maintenance Office : Report Analytics</td></tr>
    ...
    <tr><td class="row-line"><span class="label">Review Cycle:</span> Monthly</td><td class="row-line"><span class="label">Effectivity Date:</span></td></tr>
    </table>
    ...
    <table><thead><tr><th>#</th><th>Report ID</th><th>Date</th><th>Status</th><th>Building</th><th>Concern</th><th>College</th><th>Floor</th><th>Room</th><th>Email</th><th>Type</th></tr></thead><tbody>${rowsHtml || '<tr><td colspan="11">No data.</td></tr>'}</tbody></table>
  ...
  showConfirm("Archive this report? This will notify the reporter.", async () => {
    try {
      ...
      // Get all reports in the same group
      const group = reports.filter(r => getGroupKey(r) === getGroupKey(selectedReport));
      
      // Update each report in the group to Archived status
      const updated = await Promise.all(group.map(async (r) => {
        const res = await fetch(`${API_BASE}/api/reports/${r._id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ 
            status: "Archived",  // Use string literal "Archived", not statusValue
            sendEmail: sendEmail 
          }),
        });
      ...
      setReports(prev => prev.map(r => {
        const found = updated.find(u => u._id === r._id);
        return found || r;
      }));
      
      // Update selected report
      const archivedReport = updated.find(u => u._id === selectedReport._id) || updated[0];
      setSelectedReport(archivedReport);
            ...
            <div className="email-toggle-section" style={{ marginTop: "8px", padding: "8px 12px", background: "#f0fdf4", borderRadius: "8px", border: "1px solid #bbf7d0" }}>
              <label className="email-toggle-label" style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer" }}>
                ...
                <span style={{ fontSize: "0.75rem", fontWeight: "500", color: "#166534" }}>
                  📧 Notify student via email when updating status or adding comment
                </span>
              </label>
            </div>
      ...
      <div className="report-wrapper">
        <div className="header">
          ...
          <div className="header-actions">
            ...
            <button className="printreports-btn" onClick={() =>   executePrint(signatories)} type="button">
              <IconPrint /> Print Reports
            </button>
          </div>
        </div>
