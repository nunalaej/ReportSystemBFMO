// This file is identical to app/Admin/Reports/page.tsx (document 10)
// with ONE addition: an inline Concern Type correction select inside the report modal.
//
// SEARCH FOR: /* ── CONCERN CORRECTION ── */
// That marks the new block added inside modalContent > modal-meta-grid.
...
/* ── Concern type options (admin can correct any report) ── */
const CONCERN_TYPES = ["Civil", "Electrical", "Mechanical", "Safety Hazard", "Other"];
...
const formatConcernLabel = formatConcern;
...
function getStepState(stepName: string, current: string|undefined, ms: MetaStatus[]): "completed"|"active"|"pending" {
  ...
  const ci=flow.findIndex(s=>s.name===(current||"Pending")); const si=flow.findIndex(s=>s.name===stepName);
...
const FALLBACK_STATUSES: MetaStatus[] = [
  ...
  {id:"3",name:"In Progress",color:"#4169E1"},{id:"4",name:"Resolved",color:"#28A745"},
...
async function autoCloseStaleResolved(
  reports: Report[], apiBase: string,
  onClosed: (updated: Report[]) => void
) {
  const now=Date.now(); const cutoff=AUTO_CLOSE_DAYS*24*60*60*1000;
  const stale=reports.filter(r=>{
    if (r.status!=="Resolved") return false;
    const ts=r.updatedAt||r.createdAt; if (!ts) return false;
    return now-new Date(ts).getTime()>=cutoff;
  });
  if (!stale.length) return;
  const closed: Report[]=[];
  await Promise.allSettled(stale.map(async r=>{
    try {
      const res=await fetch(`${apiBase}/api/reports/${r._id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({status:"Closed",sendEmail:false})});
      const data=await res.json().catch(()=>null);
      if (!res.ok||!data?.success) return;
      await fetch(`${apiBase}/api/reports/${r._id}/comments`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:"[Auto-closed] Report was resolved for 7 days with no response from the reporter.",by:"System",sendEmail:false})});
      closed.push({...r,status:"Closed"});
    } catch {}
  }));
...
export default function ReportPage() {
  ...
  const DEFAULT_SIGNATORIES: Signatory[] = [
    {name:"",role:"Prepared by"},{name:"",role:"Reviewed by"},{name:"",role:"Approved by"},
  ];
  ...
  /* ── Body scroll lock ── */
  useEffect(()=>{
    document.body.style.overflow=(selectedReport||showTaskModal||showProgress||showPrintModal)?"hidden":"";
    return ()=>{document.body.style.overflow="";};
  },[selectedReport,showTaskModal,showProgress,showPrintModal]);
  ...
  },[isImageExpanded,showProgress,showTaskModal,showPrintModal,selectedReport]),
    !!(selectedReport||isImageExpanded||showTaskModal||showProgress||showPrintModal)
  );
  ...
  const fetchReports=useCallback(async()=>{
    try {
      ...
      await autoCloseStaleResolved(list,API_BASE,closedReports=>{
        setReports(prev=>prev.map(r=>{const f=closedReports.find(c=>c._id===r._id);return f?{...r,status:"Closed"}:r;}));
        showToast(`${closedReports.length} report${closedReports.length>1?"s":""} auto-closed after 7 days with no response.`,"info");
      });
  ...
  const renderStatusPill=useCallback((statusRaw?:string)=>{
    const status=statusRaw||"Pending"; const color=getStatusColor(status);
    return <span className="status-pill" style={{backgroundColor:color,color:"#fff",textShadow:"0 1px 2px rgba(0,0,0,0.2)",padding:"4px 8px",borderRadius:"12px",fontSize:"0.75rem",fontWeight:"500",display:"inline-block",lineHeight:"1"}}>{status}</span>;
  },[getStatusColor]);
  const getExistingTask=useCallback((r:Report):ExistingTask|null=>{
    const rid=r.reportId||r._id; return existingTasks[rid]||existingTasks[r._id]||null;
  },[existingTasks]);
  ...
  /* ── Filter options ── */
  const buildingOptions=["All Buildings",...new Set(reports.filter(r=>(concernFilter==="All Concerns"||r.concern===concernFilter)&&statusMatchesFilter(r.status,statusFilter)).map(r=>r.building).filter((v):v is string=>Boolean(v)))];
  ...
  },[buildingFilter,statusFilter,reports,concernFilter]);
  useEffect(()=>{
    const valid=new Set(reports.filter(r=>(concernFilter==="All Concerns"||r.concern===concernFilter)&&statusMatchesFilter(r.status,statusFilter)).map(r=>r.building));
    if (buildingFilter!=="All Buildings"&&!valid.has(buildingFilter)) setBuildingFilter("All Buildings");
  },[concernFilter,statusFilter,reports,buildingFilter]);
  ...
  const filteredReports=reportsToDisplay.filter(r=>{
    ...
    const rCollege=collegeRaw.includes(" - ")?collegeRaw.split(" - ")[0].trim():collegeRaw;
  ...
  const executePrint=useCallback((sigs:Signatory[])=>{
    ...
    const concernCounts=new Map<string,number>(); const buildingCounts=new Map<string,number>();
    filteredReports.forEach(r=>{
      const ck=formatConcernLabel(r)||"Unspecified"; concernCounts.set(ck,(concernCounts.get(ck)||0)+1);
      const bk=(r.building||"Unspecified").trim(); buildingCounts.set(bk,(buildingCounts.get(bk)||0)+1);
    });
    const total=filteredReports.length;
    const statusSummaryHtml=Object.entries(statusCounts).filter(([,c])=>c>0)
      .map(([l,c])=>`<li>${l}: ${c} (${total>0?Math.round((c/total)*100):0}%)</li>`).join("")||"<li>No data.</li>";
    const safe=(v?:string)=>v?String(v):"";
    const rowsHtml=filteredReports.map((r,i)=>`<tr><td>${i+1}</td><td>${safe(r.reportId)}</td><td>${r.createdAt?new Date(r.createdAt).toLocaleString():""}</td><td>${safe(r.status)}</td><td>${safe(r.building)}</td><td>${safe(formatConcernLabel(r))}</td><td>${safe(r.college)}</td><td>${safe(r.floor)}</td><td>${safe(r.room)}</td><td>${safe(r.email)}</td><td>${safe(r.userType)}</td></tr>`).join("");
    const printedDate=new Date().toLocaleString();
    const sigBlocksHtml=sigs.map(s=>`<div class="sig-block"><div class="sig-line"></div><div class="sig-name">${s.name||"Signature over Printed Name"}</div><div class="sig-role">${s.role}</div></div>`).join("");
    const html=`<!doctype html><html><head><meta charset="utf-8"/><title>BFMO Analytics</title><style>body{font-family:system-ui,sans-serif;font-size:8px;color:#111827;padding:10px}.doc-table{width:100%;border-collapse:collapse}.logo-cell{width:90px;text-align:center}.logo-cell img{width:64px;height:64px;padding-top:12px;object-fit:contain}.title{font-size:14px;font-weight:700;color:#fff;background:#029006;padding:8px}.row-line{border-bottom:1px solid #000;padding-bottom:4px}.label{font-weight:600}h1{font-size:18px;margin:16px 0 4px}h2{font-size:15px;margin-top:16px;margin-bottom:4px}.meta{font-size:11px;color:#374151;margin-bottom:12px}table{width:100%;border-collapse:collapse;margin-top:8px}th,td{border:1px solid #d1d5db;padding:4px 6px;text-align:left}thead{background:#f3f4f6}ul{margin:4px 0 8px 16px;padding:0}li{margin:2px 0}.sig-row{display:flex;justify-content:space-around;gap:24px;flex-wrap:wrap;margin-top:48px}.sig-block{flex:1;min-width:140px;max-width:200px;text-align:center}.sig-line{border-top:1px solid #111827;margin-top:40px;margin-bottom:4px}.sig-name{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.04em}.sig-role{font-size:8px;color:#6b7280;margin-top:2px}@media print{*{-webkit-print-color-adjust:exact!important}}</style></head><body><table class="doc-table"><tr><td class="logo-cell" rowspan="4"><img src="/logo-dlsud.png"/></td><td colspan="2" class="title">Building Facilities Maintenance Office : Report Analytics</td></tr><tr><td class="row-line"><span class="label">Printed:</span> ${printedDate}</td><td class="row-line"><span class="label">Confidentiality:</span> Research Purpose</td></tr><tr><td class="row-line"><span class="label">Review Cycle:</span> Monthly</td><td class="row-line"><span class="label">Effectivity Date:</span></td></tr></table><h1>BFMO Analytics — Tabular Report</h1><div class="meta">Records: ${total}</div><h2>Status Summary</h2><ul>${statusSummaryHtml}</ul><h2>By Concern</h2><ul>${[...concernCounts.entries()].sort((a,b)=>b[1]-a[1]).map(([n,c])=>`<li>${n}: ${c}</li>`).join("")||"<li>None.</li>"}</ul><h2>By Building</h2><ul>${[...buildingCounts.entries()].sort((a,b)=>b[1]-a[1]).map(([n,c])=>`<li>${n}: ${c}</li>`).join("")||"<li>None.</li>"}</ul><h2>Detailed Report</h2><table><thead><tr><th>#</th><th>Report ID</th><th>Date</th><th>Status</th><th>Building</th><th>Concern</th><th>College</th><th>Floor</th><th>Room</th><th>Email</th><th>Type</th></tr></thead><tbody>${rowsHtml||'<tr><td colspan="11">No data.</td></tr>'}</tbody></table><div class="sig-row">${sigBlocksHtml}</div><script>window.onload=()=>{window.print();setTimeout(()=>window.close(),500);}<\/script></body></html>`;
  ...
  const syncComments=async(updated:Comment[])=>{
    ...
    try {
      setSaving(true);
      const res=await fetch(`${API_BASE}/api/reports/${selectedReport._id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({status:selectedReport.status||"Pending",comments:updated,overwriteComments:true})});
      const data=await res.json().catch(()=>null);
      if (!res.ok||!data?.success) throw new Error(data?.message||"Failed");
      const up=data.report as Report;
      setReports(p=>p.map(r=>r._id===up._id?up:r)); setSelectedReport(up);
      setEditingIndex(null); setEditingText(""); showToast("Comment updated.","success");
  ...
  const handleSaveChanges=async()=>{
    ...
    try {
      ...
      if (trimmed) {
        const cr=await fetch(`${API_BASE}/api/reports/${selectedReport._id}/comments`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:trimmed,by:"Admin",sendEmail})});
  ...
  const addIndividualComment=async()=>{
    if (!selectedReport) return;
    const trimmed=commentText.trim(); if (!trimmed){showToast("Please enter a comment.","error");return;}
    try {
      setSaving(true);
      const res=await fetch(`${API_BASE}/api/reports/${selectedReport._id}/comments`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:trimmed,by:"Admin",sendEmail})});
      const data=await res.json().catch(()=>null);
      if (!res.ok||!data?.success) throw new Error(data?.message||"Failed");
      const up=data.report as Report;
      setReports(p=>p.map(r=>r._id===up._id?up:r)); setSelectedReport(up); setCommentText("");
      showToast(`Comment added.${sendEmail?" Email notification sent.":""}`, "success");
    } catch(e:any){showToast(e.message||"Failed.","error");}
    finally{setSaving(false);}
  };
  ...
  const saveEditedComment=async(i:number)=>{
    ...
    const trimmed=editingText.trim(); if(!trimmed){showToast("Comment cannot be empty.","error");return;}
  ...
  const handleArchive=async()=>{
    ...
    showConfirm("Archive this report? This will notify the reporter.",async()=>{
      try {
        ...
        const group=reports.filter(r=>getGroupKey(r)===getGroupKey(selectedReport));
        const updated=await Promise.all(group.map(async r=>{
          const res=await fetch(`${API_BASE}/api/reports/${r._id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({status:"Archived",sendEmail})});
          const data=await res.json().catch(()=>null);
          if (!res.ok||!data?.success) throw new Error(data?.message||"Failed to archive report");
          return data.report as Report;
        }));
        setReports(prev=>prev.map(r=>{const f=updated.find(u=>u._id===r._id);return f||r;}));
        const ar=updated.find(u=>u._id===selectedReport._id)||updated[0];
        setSelectedReport(ar); setStatusValue("Archived");
        showToast(`Report archived.${sendEmail?" Email notification sent.":""}`, "success");
        await fetchReports();
      } catch(error:any){showToast(error.message||"Failed to archive report.","error");}
      finally{setSaving(false);}
    });
  };
  ...
  /* ── Task helpers ── */
  const viewExistingTask=(task:ExistingTask,e?:React.MouseEvent)=>{e?.stopPropagation();router.push(`/Admin/Task?highlight=${task._id}`);};
  const openCreateTask=(r:Report)=>{setTaskReport(r);setTaskName(r.heading||"");setTaskStaff([]);setTaskStaffInput("");setTaskPriority(metaPriorities[0]?.id||"");setTaskChecklist([]);setTaskCheckInput("");setTaskNotes("");setShowTaskModal(true);};
  ...
  /* ── Signatory modal ── */
  const signatoryModalContent=showPrintModal?(
    <div className="report-modal-backdrop" onClick={()=>setShowPrintModal(false)}>
      <div onClick={e=>e.stopPropagation()} style={{background:"white",borderRadius:"16px",padding:"24px",width:"90%",maxWidth:"600px",maxHeight:"80vh",overflow:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:"20px"}}>
          <h2 style={{margin:0}}>Configure Signatories</h2>
          <button onClick={()=>setShowPrintModal(false)} style={{background:"none",border:"none",fontSize:"24px",cursor:"pointer"}}>✕</button>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:"16px",marginBottom:"24px"}}>
          {signatories.map((sig,idx)=>(
            <div key={idx} style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:"12px",alignItems:"center"}}>
              <input type="text" placeholder="Full Name" value={sig.name} onChange={e=>{const u=[...signatories];u[idx].name=e.target.value;setSignatories(u);}} style={{padding:"8px 12px",border:"1px solid #ddd",borderRadius:"8px",fontSize:"14px"}}/>
              <input type="text" placeholder="Role (e.g., Head Engineer)" value={sig.role} onChange={e=>{const u=[...signatories];u[idx].role=e.target.value;setSignatories(u);}} style={{padding:"8px 12px",border:"1px solid #ddd",borderRadius:"8px",fontSize:"14px"}}/>
              <button onClick={()=>setSignatories(signatories.filter((_,i)=>i!==idx))} style={{padding:"8px 12px",background:"#fee2e2",border:"none",borderRadius:"8px",color:"#dc2626",cursor:"pointer",fontWeight:"bold"}}>Remove</button>
            </div>
          ))}
        </div>
        <button onClick={()=>setSignatories([...signatories,{name:"",role:""}])} style={{width:"100%",padding:"10px",background:"#f3f4f6",border:"1px dashed #9ca3af",borderRadius:"8px",cursor:"pointer",marginBottom:"20px",fontWeight:"500"}}>+ Add Signatory</button>
        <div style={{display:"flex",gap:"12px"}}>
          <button onClick={()=>setSignatories(DEFAULT_SIGNATORIES)} style={{flex:1,padding:"10px",background:"#f3f4f6",border:"none",borderRadius:"8px",cursor:"pointer"}}>Reset to Defaults</button>
          <button onClick={()=>{setShowPrintModal(false);executePrint(signatories);}} style={{flex:2,padding:"10px",background:"#22c55e",color:"white",border:"none",borderRadius:"8px",cursor:"pointer",fontWeight:"bold"}}>Confirm & Print</button>
        </div>
      </div>
    </div>
  ):null;
              ...
              {buildStatusFlow(metaStatuses).map((step,idx,arr)=>{
                ...
                return(
                  <div key={step.id} className={`stepper-step ${stateClass}`}>
                    ...
                    <div className="stepper-content"><div className="stepper-title">{step.name}</div><span className="stepper-status" style={state!=="pending"?{backgroundColor:step.color+"22",color:step.color}:{}}>{state==="completed"?"Completed":state==="active"?"Current":"Pending"}</span></div>
                  </div>
                );
              })}
            </div>
      ...
      <div className="report-modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="modal-content">
          ...
          <div className="modal-info">
                ...
                {correctingConcern ? (
                  ...
                  <span style={{display:"inline-flex",alignItems:"center",gap:6}}>
                    <span style={{
                      ...
                      background:CONCERN_COLORS[selectedReport.concern||""]?.bg||"#f3f4f6",
                      color:CONCERN_COLORS[selectedReport.concern||""]?.color||"#6b7280",
                    }}>
            ...
            <div className="email-toggle-section" style={{marginTop:"8px",padding:"8px 12px",background:"#f0fdf4",borderRadius:"8px",border:"1px solid #bbf7d0"}}>
              <label className="email-toggle-label" style={{display:"flex",alignItems:"center",gap:"8px",cursor:"pointer"}}>
                ...
                <span style={{fontSize:"0.75rem",fontWeight:"500",color:"#166534"}}>📧 Notify student via email when updating status or adding comment</span>
              </label>
            </div>
            <div className="comments-section">
              ...
              <div className="modal-actions">
                ...
                {selectedReport.status!=="Archived"&&<button className="archive-btn" onClick={handleArchive} disabled={saving} type="button">Archive report</button>}
          ...
          <div className="task-modal-right">
            <div className="task-field-group">
              ...
              <div className="task-staff-input-row">
                {metaStaff.length>0?(<select className="task-input" value={taskStaffInput} onChange={e=>setTaskStaffInput(e.target.value)}><option value="">-- Select staff --</option>{metaStaff.filter(s=>!taskStaff.includes(s.name)&&(s.disciplines.length===0||s.disciplines.some(d=>d.toLowerCase()===(taskReport?.concern||"").toLowerCase()))).map(s=><option key={s.name} value={s.name}>{s.name}</option>)}</select>)
            ...
            <div className="task-field-group">
              ...
              <div className="task-staff-input-row"><input type="text" className="task-input" value={taskCheckInput} placeholder="Add a checklist step…" onChange={e=>setTaskCheckInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();addChecklistItem();}}}/><button type="button" className="task-add-btn" onClick={addChecklistItem} disabled={!taskCheckInput.trim()}>Add</button></div>
            </div>
          </div>
        </div>
  ...
  const renderReportCard=(report:Report,showDupLink=false)=>{
    ...
    const existing=getExistingTask(report); const statusColor=getStatusColor(report.status);
    const cc=CONCERN_COLORS[report.concern||""]||CONCERN_COLORS["Other"];
    return(
      <div key={report._id} className="report" onClick={()=>handleCardClick(report)}>
        ...
        <div className="report-img-container"><img src={resolveImg(report.image||report.ImageFile)} alt="Report" className="report-img" onError={e=>{(e.target as HTMLImageElement).src=defaultImg;}}/></div>
        <div className="report-body">
          ...
          <div className="status-focus-row" style={{backgroundColor:`${statusColor}10`,borderLeft:`3px solid ${statusColor}`,padding:"6px 8px",borderRadius:"4px",marginBottom:"8px"}}>
            <span className="status-focus-label">Status</span>{renderStatusPill(report.status)}
          </div>
          {report.concern&&<span style={{display:"inline-block",padding:"2px 8px",borderRadius:999,fontSize:"0.7rem",fontWeight:700,background:cc.bg,color:cc.color,marginBottom:6}}>{report.concern}</span>}
          ...
          <div className="report-card-actions" onClick={e=>e.stopPropagation()}>
            <button type="button" className="card-progress-btn" onClick={e=>openProgress(report,e)}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>Progress</button>
  ...
  const renderReportRow=(report:Report)=>{
    const sColor=getStatusColor(report.status); const existing=getExistingTask(report);
    return(
      <div key={report._id} className="report-list-row" onClick={()=>handleCardClick(report)}>
        ...
        <div className="report-list-actions" onClick={e=>e.stopPropagation()}>
          <button type="button" className="card-progress-btn" style={{fontSize:"0.68rem"}} onClick={e=>openProgress(report,e)}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg></button>
  ...
  return(
    <>
      <div className="report-wrapper">
        <div className="header">
          ...
          <div className="header-actions">
            ...
            <button className="printreports-btn" onClick={()=>executePrint(signatories)} type="button"><IconPrint/> Print Reports</button>
          </div>
        </div>
        ...
        <div className="filters-card">
          ...
          <div className="date-filter-row">
            ...
            <div className="date-filter-tabs">
              {(["all","today","week","month","custom"] as DateFilter[]).map(f=>(<button key={f} type="button" className={`date-filter-tab${dateFilter===f?" date-filter-tab--active":""}`} onClick={()=>setDateFilter(f)}>{f==="all"?"All time":f==="today"?"Today":f==="week"?"Last 7 days":f==="month"?"Last 30 days":"Custom"}</button>))}
            </div>
            {dateFilter==="custom"&&<div className="date-custom-inputs"><input type="date" className="date-custom-input" value={customDateFrom} onChange={e=>setCustomDateFrom(e.target.value)}/><span style={{color:"#9ca3af",fontSize:13}}>to</span><input type="date" className="date-custom-input" value={customDateTo} onChange={e=>setCustomDateTo(e.target.value)}/></div>}
          </div>
        </div>
          ...
          <>
            {selectedGroup?(
              <div className={viewMode==="list"?"reports-list-view":"reports-list"}>
                <div className="group-header" style={{gridColumn:"1 / -1"}}><h2>Similar reports for <em>{selectedGroup.replace(/\|/g," › ")}</em></h2><button onClick={()=>setSelectedGroup(null)} className="back-btn" type="button">← Back</button></div>
              ...
              <>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                  <span style={{fontSize:"0.8rem",color:"#6b7280"}}>{filteredReports.length} report{filteredReports.length!==1?"s":""}{dateFilter!=="all"&&` · ${dateFilter==="today"?"Today":dateFilter==="week"?"Last 7 days":dateFilter==="month"?"Last 30 days":"Custom range"}`}</span>
                </div>
                {viewMode==="list"?(<div className="reports-list-view"><div className="report-list-header"><span>Report</span><span>Status</span><span>Building</span><span>Concern</span><span>College</span><span>Date</span><span>Actions</span></div>{paginatedReports.map(r=>renderReportRow(r))}</div>):(<div className="reports-list">{paginatedReports.map(r=>renderReportCard(r,true))}</div>)}
                {totalPages>1&&(
      ...
      {mounted&&signatoryModalContent&&createPortal(signatoryModalContent,document.body)}
