...
/**
 * Strip the year-level suffix from a college string.
 * "CICS - 1st Year" → "CICS"
 * "COCS"            → "COCS"
 */
const stripYearFromCollege = (raw: string): string => {
  if (!raw) return raw;
...
const normalizeStatus = (status: string | undefined, statuses: MetaStatus[]) => {
  ...
  if (s.includes("inspecting"))                          { const h = statuses.find(st => /inspecting/i.test(st.name));  if (h) return h.name; }
  if (s.includes("in progress") || s.includes("inprogress")) { const h = statuses.find(st => /progress/i.test(st.name));  if (h) return h.name; }
  if (s.includes("unfinished"))                          { const h = statuses.find(st => /unfinished/i.test(st.name)); if (h) return h.name; }
  if (s.includes("closed"))                              { const h = statuses.find(st => /closed/i.test(st.name));     if (h) return h.name; }
  if (s.includes("archived"))                            { const h = statuses.find(st => /archived/i.test(st.name));   if (h) return h.name; }
  return "Pending";
};
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div style={{ background:"var(--surface-raise)", border:"1px solid var(--card-border-2)", borderRadius:12, padding:"10px 14px", boxShadow:"var(--shadow-lg)", fontFamily:"var(--font-sans)" }}>
      {label && <p style={{ margin:"0 0 4px", fontSize:11, color:"var(--muted)", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.06em" }}>{label}</p>}
      {payload.map((p: any, i: number) => (
        <p key={i} style={{ margin:0, fontSize:13, color:"var(--text)", fontWeight:600, fontFamily:"var(--font-mono)" }}>
          <span style={{ display:"inline-block", width:8, height:8, borderRadius:2, background:p.color, marginRight:8, verticalAlign:"middle" }}/>
          {p.value}
          <span style={{ color:"var(--muted)", fontWeight:400, marginLeft:6, fontFamily:"var(--font-sans)" }}>{p.name !== "value" ? p.name : "reports"}</span>
        </p>
      ))}
    </div>
  );
...
/* =====================================================================
   STAT CARD
   ===================================================================== */
const StatCard: FC<{ label: string; value: string|number; color: string; icon: React.ReactNode; sub?: string; }> = ({ label, value, color, icon, sub }) => (
  <div className="stat-card-v2">
    <div className="stat-card-v2__accent" style={{ background:`linear-gradient(90deg, ${color}, transparent)` }} />
    <div className="stat-card-v2__icon" style={{ background:`${color}18`, border:`1px solid ${color}35`, color }}>{icon}</div>
...
const Analytics: FC = () => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    } catch { console.warn("[Analytics] Tasks fetch failed"); }
  ...
  useEffect(() => {
    ...
    return () => { clearInterval(interval); window.removeEventListener("focus", fetchTasks); };
  }, [canView, fetchTasks]);
    ...
    const load = async () => {
      try {
        ...
        if (data) {
          ...
          if (Array.isArray(data.hiddenStudentStatuses)) setHiddenStudentStatuses(data.hiddenStudentStatuses);
        }
      } catch (err) { console.warn("[Analytics] Meta fetch failed:", err); }
    };
    void load();
  }, []);
  ...
  // Only exclude "Archived" from the default selection — all other statuses
  // (including Closed, Cancelled, or any future ones) are checked by default.
  const DEFAULT_STATUS_SET = useMemo(
    () => new Set(statuses.filter(s => s.name.toLowerCase() !== "archived").map(s => s.name)),
    [statuses]
  );

  // When statuses load from the API, sync selectedStatuses so new statuses
  // (Closed, Cancelled, or any admin-added ones) are automatically checked.
  useEffect(() => {
  ...
  /* ── Filters ── */
  // Initial state: all default statuses selected except Archived
  const [selectedStatuses,  setSelectedStatuses]  = useState<Set<string>>(
    () => new Set(DEFAULT_STATUSES.filter(s => s.name.toLowerCase() !== "archived").map(s => s.name))
  );
  ...
  const clearAllFilters = () => {
    setSelectedStatuses(new Set(DEFAULT_STATUS_SET));
    setSelectedBuildings(new Set()); setSelectedConcerns(new Set()); setSelectedColleges(new Set());
  ...
  const filtered = useMemo(() => {
    ...
    return reports.filter(r => {
      ...
      // College filter compares against the stripped (no-year) college name
      if (selectedColleges.size  && !selectedColleges.has(stripYearFromCollege(r.college || ""))) return false;
  ...
  const reportsMatchingStatus = useMemo(() => {
    return reports.filter(r => {
      const ns = normalizeStatus(r.status, statuses);
      return !!ns && selectedStatuses.has(ns);
    });
  }, [reports, selectedStatuses, statuses]);

  /* ── Filter option lists — college stripped of year ── */
  ...
  const availableConcerns = useMemo(() => {
    const s = new Set<string>();
    reportsMatchingStatus.forEach(r => { const b = getBaseConcernFromReport(r); if (b) s.add(b); });
    selectedConcerns.forEach(v => s.add(v));
    return [...s].sort();
  }, [reportsMatchingStatus, selectedConcerns]);

  /** College chips show only the college name — year stripped */
  const availableColleges = useMemo(() => {
    const s = new Set<string>();
    reportsMatchingStatus.forEach(r => {
      const stripped = stripYearFromCollege((r.college || "").trim());
  ...
  const statusCounts = useMemo(() => {
    ...
    filtered.forEach(r => {
      const ns = normalizeStatus(r.status || "Pending", statuses);
      if (ns && map[ns] !== undefined) map[ns]++;
    });
    return map;
  }, [filtered, statuses]);
  ...
  const statusPieData = useMemo(() => {
    const total = Object.entries(statusCounts).filter(([name]) => selectedStatuses.has(name)).reduce((sum,[,c]) => sum+c, 0);
    return Object.entries(statusCounts)
      .filter(([name]) => selectedStatuses.has(name) && statusCounts[name] > 0)
      .map(([name, value]) => ({ name, value, color: STATUS_COLORS_MAP[name]||"#6C757D", percent: total>0?Math.round((value/total)*100):0 }));
  ...
  /**
   * College chart — strip year level so "CICS - 1st Year" and "CICS - 2nd Year"
   * both aggregate under "CICS".
   */
  const collegeData = useMemo(() => {
    return agg(filtered, r => stripYearFromCollege((r.college||"Unspecified").trim()));
  }, [filtered, agg]);
  ...
  const timeSeriesData: TimeSeriesPoint[] = useMemo(() => {
    ...
    filtered.forEach(r => {
      ...
      if (timeMode==="day") {
        ...
        label=`Wk ${tmp.getFullYear().toString().slice(2)}-${tmp.getMonth()+1}`; sortKey=tmp.getTime();
  ...
  const executePrint = useCallback((sigs: Signatory[]) => {
    ...
    const rowsHtml = filtered.map((r,i) =>
      `<tr><td>${i+1}</td><td>${safe(r.reportId)}</td><td>${r.createdAt?new Date(r.createdAt).toLocaleString():""}</td><td>${safe(r.status)}</td><td>${safe(r.building)}</td><td>${safe(formatConcernLabel(r))}</td><td>${safe(stripYearFromCollege(r.college||""))}</td><td>${safe(r.floor)}</td><td>${safe(r.room)}</td><td>${safe(r.email)}</td><td>${safe(r.userType)}</td></tr>`
    ).join("");
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      setServerTasks(Array.isArray(data)?data:Array.isArray(data.tasks)?data.tasks:[]);
  ...
  const createTask = useCallback(async () => {
    ...
    try {
      ...
      setServerTasks(prev=>[data.task,...prev]); setCreateDraft(EMPTY_DRAFT); setStaffInput(""); setCheckInput(""); setShowCreateModal(false); showStatus("✅ Task created!");
  ...
  const updateTask = useCallback(async (taskId: string, updates: Partial<Task>) => {
    try {
      ...
      const data=await res.json().catch(()=>null);
      if (!res.ok||!data?.success) throw new Error(data?.message||"Failed.");
      setServerTasks(prev=>prev.map(t=>t._id===taskId?data.task:t)); setEditingTaskId(null); showStatus("✅ Updated!");
  ...
  const deleteTask = useCallback(async (taskId: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed.");
      setServerTasks(prev=>prev.filter(t=>t._id!==taskId)); if (editingTaskId===taskId) setEditingTaskId(null); showStatus("✅ Deleted!");
  ...
  const toggleChecklistItem = useCallback(async (task: Task, item: ChecklistItem, done: boolean) => {
    ...
    } catch { setServerTasks(prev=>prev.map(t=>t._id===task._id?{...t,checklist:task.checklist}:t)); showStatus("❌ Checklist update failed"); }
  }, []);
  ...
  const resolvedClosedCount = useMemo(
    () => (statusCounts["Resolved"]||0) + (statusCounts["Closed"]||0),
    [statusCounts]
  );
  const resolvedPct = useMemo(
    () => filtered.length>0 ? Math.round((resolvedClosedCount/filtered.length)*100) : 0,
    [filtered, resolvedClosedCount]
  );

  const getStatusIcon = (statusName: string): React.ReactNode => {
    const lower = statusName.toLowerCase();
    if (lower.includes("pending"))    return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4"><circle cx="12" cy="12" r="9"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>;
    if (lower.includes("inspect"))    return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>;
    if (lower.includes("progress"))   return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4"><circle cx="12" cy="12" r="9"/><polyline points="12 6 12 12 16 14"/></svg>;
    if (lower.includes("resolved"))   return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4"><polyline points="20 6 9 17 4 12"/></svg>;
    if (lower.includes("unfinished")) return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4"><circle cx="12" cy="12" r="9"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>;
    if (lower.includes("closed"))     return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="9" x2="15" y2="15"/><line x1="15" y1="9" x2="9" y2="15"/></svg>;
  ...
  if (!isLoaded || !canView) return (
    <div className="analytics-wrapper" style={{ display:"flex", alignItems:"center", justifyContent:"center", minHeight:"100vh" }}>
      <div style={{ textAlign:"center" }}><div className="analytics-spinner"/><p style={{ color:"var(--muted)", fontSize:14, marginTop:14 }}>Checking permissions…</p></div>
    </div>
  );
      ...
      <div className="analytics-container">
        ...
        <div className="analytics-header">
          ...
          <div className="header-actions">
            ...
            <button className="small-btn" onClick={() => redirect('/Admin/Task')}>
              <IconTasks /> Tasks
            </button>
        ...
        {filtersOpen && (
          <div className="card filters" style={{ marginBottom:20 }}>
            <div className="filters-row">
              ...
              <div className="filter-block">
                <h4>College</h4>
                {/* Year level stripped — shows CICS, COCS etc. without "- 1st Year" */}
              ...
              <div className="filter-block">
                ...
                <div className="quick-dates">
                  <button className="quick-date-btn" onClick={()=>setLastDays(7)}>7d</button>
                  <button className="quick-date-btn" onClick={()=>setLastDays(30)}>30d</button>
                  <button className="quick-date-btn" onClick={()=>setLastDays(90)}>90d</button>
                  <button className="quick-date-btn" onClick={()=>{setDateFrom("");setDateTo("");}}>All</button>
                </div>
              </div>
            </div>
            <div className="filters-actions">
              <span className="muted">Showing <strong>{filtered.length}</strong> filtered reports</span>
              <button className="small-btn" onClick={clearAllFilters}>Clear all filters</button>
            </div>
          </div>
        )}
        ...
        <div className="stat-grid">
          ...
          {Object.entries(statusCounts)
            ...
            .map(([name, count]) => (
              <StatCard
                key={name} label={name} value={count}
                color={STATUS_COLORS_MAP[name]||"#6C757D"}
                sub={name.toLowerCase().includes("pending")?"Needs attention":name.toLowerCase().includes("inspect")?"Under inspection":name.toLowerCase().includes("progress")?"Work in progress":name.toLowerCase().includes("resolved")?`${filtered.length>0?Math.round((count/filtered.length)*100):0}% of total`:name.toLowerCase().includes("unfinished")?"Unfinished reports":name.toLowerCase().includes("closed")?"Completed & closed":""}
                icon={getStatusIcon(name)}
              />
          ))}
          <StatCard
            label="Resolution Rate" value={`${resolvedPct}%`} color="#22d3ee"
            sub={`${resolvedClosedCount}/${filtered.length} resolved+closed`}
          ...
          <div className="card">
            ...
            <div className="chart-wrap" style={{ height:260 }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={statusPieData} cx="50%" cy="50%" innerRadius="58%" outerRadius="85%" paddingAngle={2} dataKey="value">
            ...
            <div className="legend-grid">
              {statusPieData.map(e=>(
                <div key={e.name} className="legend-item">
                  <span className="legend-label"><span className="legend-swatch" style={{background:e.color}}/>{e.name}</span>
            ...
            <div className="bar-wrap" style={{ height:260 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={buildingData} barSize={22} margin={{top:10,right:10,left:-20,bottom:0}}>
                  ...
                  <Bar dataKey="value" radius={[6,6,0,0]}>{buildingData.map((e,i)=><Cell key={e.name} fill={getBuildingColor(e.name,i)}/>)}</Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div className="bar-wrap" style={{ height:260 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={concernData} barSize={20} margin={{top:10,right:10,left:-20,bottom:0}}>
                  ...
                  <XAxis dataKey="name" tick={{fontSize:10}} axisLine={false} tickLine={false}/>
                  <YAxis allowDecimals={false} axisLine={false} tickLine={false}/>
                  <Tooltip content={<CustomTooltip/>} cursor={{fill:"rgba(148,163,184,0.06)"}} labelFormatter={(_,p)=>{const pl=(p&&p[0]?.payload) as ConcernChartDatum|undefined;return pl?pl.name:"";}} formatter={(v:any)=>[`${v}`,"Reports"]}/>
                  <Bar dataKey="value" radius={[6,6,0,0]}>{concernData.map(e=><Cell key={`${e.base}-${e.name}`} fill={getConcernColorFromBase(e.base)}/>)}</Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div className="legend-grid" style={{gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))"}}>
              {["Civil","Mechanical","Electrical","Safety Hazard","Other"].filter(b=>concernData.some(d=>d.base===b)).map(b=>(
                <div key={b} className="legend-item"><span className="legend-label"><span className="legend-swatch" style={{background:getConcernColorFromBase(b)}}/>{b}</span></div>
              ))}
            </div>
          </div>

          {/* College horizontal bar — year stripped */}
          <div className="card">
            ...
            <div className="bar-wrap" style={{ height:260 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={collegeData} layout="vertical" barSize={14} margin={{top:10,right:20,left:10,bottom:0}}>
                  ...
                  <Tooltip content={<CustomTooltip/>} cursor={{fill:"rgba(148,163,184,0.06)"}}/>
                  <Bar dataKey="value" radius={[0,6,6,0]}>{collegeData.map((e,i)=><Cell key={e.name} fill={getCollegeColor(e.name,i)}/>)}</Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div className="chart-wrap" style={{ height:300 }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={timeSeriesData} margin={{top:10,right:20,left:-15,bottom:0}}>
                  <defs><linearGradient id="areaGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#10a373" stopOpacity={0.35}/><stop offset="100%" stopColor="#10a373" stopOpacity={0}/></linearGradient></defs>
                  <CartesianGrid strokeDasharray="3 4" vertical={false}/>
          ...
          <div className="card analytics-card-full">
            <div className="resolution-head">
              <div>
                ...
                <p className="muted" style={{margin:"4px 0 0"}}>{resolvedClosedCount} resolved + closed of {filtered.length} total</p>
              </div>
            ...
            <div className="status-breakdown">
              {Object.entries(statusCounts).map(([name,count])=>(
                <div key={name} className="status-breakdown__item">
                  <div className="status-breakdown__value" style={{color:STATUS_COLORS_MAP[name]||"#6C757D"}}>{count}</div>
          ...
          <div className="create-task-modal" style={{maxWidth:580}} onClick={e=>e.stopPropagation()}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
              <div>
                ...
                <p className="create-task-modal__hint" style={{margin:"4px 0 0",fontSize:12,color:"var(--muted)"}}>Names will appear at the bottom of the printed report</p>
              </div>
              <button className="sidepanel-close" onClick={()=>setShowPrintModal(false)} style={{background:"none",border:"none",fontSize:24,cursor:"pointer",color:"var(--text-2)",padding:"4px 8px",lineHeight:1}}>×</button>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:12,maxHeight:"50vh",overflowY:"auto",paddingRight:4}}>
              {signatories.map((sig,idx)=>(
                <div key={idx} style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:10,alignItems:"end"}}>
                  <div style={{margin:0}}>
                    <label style={{fontSize:11,fontWeight:600,color:"var(--text-2)",marginBottom:4,display:"block"}}>Name</label>
                    <input placeholder="e.g. Juan dela Cruz" value={sig.name} onChange={e=>setSignatories(prev=>prev.map((s,i)=>i===idx?{...s,name:e.target.value}:s))} style={{width:"100%",padding:"8px 12px",borderRadius:8,border:"1px solid var(--border)",background:"var(--surface)",color:"var(--text)",fontSize:13,fontFamily:"inherit",boxSizing:"border-box"}}/>
                  </div>
                  <div style={{margin:0}}>
                    <label style={{fontSize:11,fontWeight:600,color:"var(--text-2)",marginBottom:4,display:"block"}}>Role</label>
                    <input placeholder="e.g. Head Engineer" value={sig.role} onChange={e=>setSignatories(prev=>prev.map((s,i)=>i===idx?{...s,role:e.target.value}:s))} style={{width:"100%",padding:"8px 12px",borderRadius:8,border:"1px solid var(--border)",background:"var(--surface)",color:"var(--text)",fontSize:13,fontFamily:"inherit",boxSizing:"border-box"}}/>
                  </div>
                  <button type="button" onClick={()=>setSignatories(prev=>prev.filter((_,i)=>i!==idx))} style={{width:40,padding:"8px 0",background:"none",border:"1px solid var(--border)",borderRadius:6,cursor:"pointer",color:"var(--text-2)",fontSize:16,transition:"all 0.14s"}} onMouseEnter={e=>{e.currentTarget.style.background="#fee2e2";e.currentTarget.style.color="#dc2626";}} onMouseLeave={e=>{e.currentTarget.style.background="none";e.currentTarget.style.color="var(--text-2)";}}>×</button>
                </div>
              ))}
            </div>
            {signatories.length>0&&(
              <div style={{marginTop:20,padding:"16px",background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:12}}>
                <p style={{margin:"0 0 12px",fontSize:11,fontWeight:700,color:"#0369a1",textTransform:"uppercase",letterSpacing:"0.05em"}}>Preview</p>
                <div style={{display:"flex",gap:16,justifyContent:"space-around",flexWrap:"wrap"}}>
                  {signatories.map((sig,idx)=>(
                    <div key={idx} style={{textAlign:"center",minWidth:100,flex:1}}>
                      ...
                      <p style={{margin:0,fontSize:12,fontWeight:700,color:"#0c4a6e"}}>{sig.name||<span style={{fontStyle:"italic",color:"#64748b",fontWeight:400}}>Signature over Printed Name</span>}</p>
                      <p style={{margin:"4px 0 0",fontSize:10,color:"#475569"}}>{sig.role||<span style={{fontStyle:"italic"}}>Role</span>}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{display:"flex",gap:8,marginTop:20}}>
              <button type="button" onClick={()=>setSignatories(prev=>[...prev,{name:"",role:""}])} style={{flex:1,padding:"8px 12px",background:"none",border:"1px dashed var(--brand)",borderRadius:8,cursor:"pointer",color:"var(--brand)",fontSize:12,fontWeight:600,fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Add Signatory
              </button>
            </div>
            <div style={{display:"flex",gap:8,marginTop:12}}>
              <button style={{flex:1,padding:"9px 12px",background:"var(--surface-2)",border:"1px solid var(--border)",borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"inherit",color:"var(--text-2)"}} onClick={()=>setSignatories(DEFAULT_SIGNATORIES)}>Reset to Defaults</button>
              <button style={{flex:2,padding:"9px 12px",background:"#10a373",border:"none",borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"inherit",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",gap:6}} onClick={()=>{setShowPrintModal(false);executePrint(signatories);}}>
